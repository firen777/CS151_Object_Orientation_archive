   ...
   public BarFrame(DataModel dataModel)
   {
      ...
      MouseAdapter mAdapt = new MouseAdapter() {
		...
		public void mousePressed(MouseEvent e) {
			...
			int i = (int) ((e.getY()) / ((barIcon.getIconHeight() * 1.0 )/ (a.size()*1.0)));
			System.out.println(i + "=" + (e.getY()) + "/" + barIcon.getIconHeight());
			double max =  (a.get(0)).doubleValue();
            for (Double v : a)
            {
               double val = v.doubleValue();
            ...
            double newVal = (e.getX()*1.0)/(barIcon.getIconWidth()*1.0) * max;
            dataModel.update(i, newVal);
		}
